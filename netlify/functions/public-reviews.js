...
exports.handler = async (event) => {
  ...
  try {
    const all = await listAllReviews(event);
    const approved = all
    ...
    return { statusCode: 200, headers, body: JSON.stringify({ ok: true, runtime: 'lambda-connected', reviews: approved }) };
  } catch (error) {
    return {
      statusCode: 500,
      headers: { ...headers, 'Cache-Control': 'no-store' },
      body: JSON.stringify({ ok: false, runtime: 'lambda-connected', error: error.message || 'Could not load reviews' })
    };
  }
};
